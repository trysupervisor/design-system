        ...
        <Message from={message.from} key={message.key}>
          {message.versions?.length && message.versions.length > 1 ? (
            <MessageBranch
              defaultBranch={0}
              key={message.key}
              onBranchChange={(branch) => setActiveBranches((current) => ({ ...current, [message.key]: branch }))}
            >
              <MessageBranchContent>
              ...
              {message.from === "assistant" && (
                <MessageToolbar>
                  ...
                  <MessageActions>
                    ...
                    <CopyAction
                      content={
                        message.versions?.[activeBranches[message.key] ?? 0]?.content || ""
                      }
                    />
                  </MessageActions>
                </MessageToolbar>
            ...
            <div key={message.key}>
              {message.attachments && message.attachments.length > 0 && (
                <Attachments className="mb-2" variant="grid">
                  {message.attachments.filter((attachment) => !removedAttachments.has(attachment.id)).map((attachment) => (
                    <Attachment
                      data={attachment}
                      key={attachment.id}
                      onRemove={() => setRemovedAttachments((current) => new Set([...current, attachment.id]))}
                    >
                      <AttachmentPreview />
                      <AttachmentRemove />
                    </Attachment>
