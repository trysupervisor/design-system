...
function themePropertiesToCss(theme: ThemeDefinition) {
  ...
  const fontFamily = theme.recipe === "ledger" && theme.font === "system-sans" ? LEDGER_NATIVE_SANS : font.cssFamily
  const monoFamily = theme.recipe === "ledger" ? LEDGER_NATIVE_MONO : DEFAULT_MONO_FAMILY
  ...
  return [
    ...
    ...(theme.recipe === "ledger"
      ? Object.entries({ ...ledgerVariables, "--ledger-font-heading": fontFamily }).map(([name, value]) => `  ${name}: ${value};`)
      : []),
  ].join("\n")
}
...
export function themeVariables(themeInput: ThemeDefinition, mode: "light" | "dark") {
  ...
  const variables: Record<string, string> = {
    "--app-font": theme.recipe === "ledger" && theme.font === "system-sans" ? LEDGER_NATIVE_SANS : `var(${font.cssVariable})`,
    "--font-sans": theme.recipe === "ledger" && theme.font === "system-sans" ? LEDGER_NATIVE_SANS : `var(${font.cssVariable})`,
    "--font-mono": theme.recipe === "ledger" ? LEDGER_NATIVE_MONO : DEFAULT_MONO_VARIABLE,
    "--spacing": `${theme.spacing * 0.25}rem`,
    "--radius": `${theme.radius}rem`,
    "--button-radius": `${theme.buttonRadius ?? theme.radius}rem`,
  ...
  if (theme.recipe === "ledger") {
    Object.assign(variables, ledgerVariables, {
      "--ledger-font-heading": theme.font === "system-sans" ? LEDGER_NATIVE_SANS : font.cssFamily,
    })
  }
  return variables
}
